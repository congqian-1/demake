-- Seed large hierarchical data for local mes database with per-batch part cap.
...
SET @parts_per_package_desired = 20; -- 目标每包件板件数
SET @max_parts_per_batch = 20000;    -- 单批次最大板件数
SET @seed_ts = UNIX_TIMESTAMP();

SET @parts_per_package = LEAST(
  @parts_per_package_desired,
  GREATEST(1, FLOOR(@max_parts_per_batch / (@boxes_per_workorder * @work_orders_per_batch)))
);

DROP PROCEDURE IF EXISTS seed_mes_hierarchy_capped;
DELIMITER $$
CREATE PROCEDURE seed_mes_hierarchy_capped()
BEGIN
...
CALL seed_mes_hierarchy_capped();
