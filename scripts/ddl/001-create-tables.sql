...
CREATE TABLE `mes_box` (
  ...
  `updated_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_batch_work_box` (`batch_num`, `work_id`, `box_code`),
...
CREATE TABLE `mes_package` (
  ...
  `updated_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_batch_work_box_package` (`batch_num`, `work_id`, `box_code`, `package_no`),
...
CREATE TABLE `mes_part` (
  ...
  `z_axis` DECIMAL(10,2) DEFAULT NULL COMMENT 'Z轴坐标',
	  ...
	  `workmanship` VARCHAR(128) DEFAULT NULL COMMENT '工艺名称',
